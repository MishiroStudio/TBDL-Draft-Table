...
function loadState() {
  try {
    ...
    state.playerCount = Math.min(
      MAX_PLAYERS,
      Math.max(MIN_PLAYERS, Number(saved.playerCount) || 4)
    );

    if (Array.isArray(saved.trainerNames)) {
      for (let i = 0; i < MAX_PLAYERS; i++) {
        if (typeof saved.trainerNames[i] === "string") {
          state.trainerNames[i] = saved.trainerNames[i];
        }
      }
    }
...
const SPRITE_ALIASES = {
  ...
  "Mega Meowstic": ["meowstic-male-mega", "meowstic-female-mega"],
...
function spriteCandidates(name) {
  ...
  if (slug.startsWith("mega-")) {
    ...
    candidates.push(`sprites/${rest}-mega.webp`);

    const match = rest.match(/^(.*)-(x|y|z)$/);
    if (match) {
      candidates.push(`sprites/${match[1]}-mega-${match[2]}.png`);
      candidates.push(`sprites/${match[1]}-mega${match[2]}.png`);
      candidates.push(`sprites/${match[1]}meg${match[2]}.png`);
    }
  }
  ...
  if (slug.startsWith("alolan-")) {
    ...
    candidates.push(`sprites/${rest}-alolan.png`);
  }

  if (slug.startsWith("galarian-")) {
    const rest = slug.slice(9);
    candidates.push(`sprites/${rest}-galar.png`);
    candidates.push(`sprites/${rest}-galarian.png`);
  }

  if (slug.startsWith("hisuian-")) {
    const rest = slug.slice(8);
    candidates.push(`sprites/${rest}-hisui.png`);
    candidates.push(`sprites/${rest}-hisuian.png`);
  }

  if (slug.startsWith("paldean-")) {
    const rest = slug.slice(8);
    candidates.push(`sprites/${rest}-paldea.png`);
...
function createSprite(name) {
  ...
  const candidates = spriteCandidates(name);
  let index = 0;
  ...
  const tryNext = () => {
    if (index >= candidates.length) {
      ...
      fallback.textContent = "Sprite fehlt";
...
function createPokemonCard(pokemon) {
  ...
  pts.className = "pokemon-points";
  pts.textContent = `${pokemon.points} Punkte`;
...
function renderBoard() {
  ...
  for (let points = 20; points >= 1; points--) {
    const column = document.createElement("div");
    column.className = "point-column";

    const head = document.createElement("div");
    head.className = "point-head";
    head.innerHTML = `${points}<small>Punkte</small>`;
...
function budgetFor(playerIndex) {
  return MAX_BUDGET - POKEMON
    .filter(p => Number(state.drafted[p.name]) === playerIndex)
...
async function draftPokemon(name, playerIndex) {
  ...
  state.drafted[name] = playerIndex;
  saveState();
  renderAll();
}
